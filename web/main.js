...
cityInput.addEventListener('input', function () {
    ...
    citySuggestTimeout = setTimeout(async () => {
        lastCityQuery = query;
        try {
            const res = await fetch(`http://localhost:8000/api/city-suggest?query=${encodeURIComponent(query)}`);
...
form.addEventListener('submit', async (e) => {
    ...
    try {
        const url = `http://localhost:8000/api/weather-stats?city=${encodeURIComponent(city)}&date=${date}`;
        ...
        document.getElementById('result-type').textContent = data.is_forecast ? 'Forecasted value' : 'Historical/statistical value';
        document.getElementById('result-type').className = data.is_forecast ? 'result-type forecast' : 'result-type historical';
